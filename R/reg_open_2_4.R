reg_open_2_4 <- function(){
  dt_scores_location <- '~/Dropbox/pkg.data/auction_emotions/Clean/dt_scores.rds'
  reg_open_2_4_location <- '~/Dropbox/pkg.data/auction_emotions/Clean/reg_open_2_4.rds'
  
  if(!file.exists(reg_open_2_4_location)){
    ...
    dt_open <- dt_scores[marker_time_elapsed <= 4 & marker_time_elapsed >= 2]
    ...
    saveRDS(l_reg, reg_open_2_4_location)
  } else {
    l_reg <- readRDS(reg_open_2_4_location)
  }
