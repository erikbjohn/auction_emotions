analysis_emotions_graphical <- function(dt_reg){
  # Next produce plots
  dt_reg <- readRDS('~/Dropbox/pkg.data/auction_emotions/Clean/dt_reg.rds')
  ...
  for(sTreatment in treatments){
    ...
    ggsave(filename=fName, device='pdf')
    # Single auction type plots
    
    
    ggplot2::ggplot(dt_sprd[auction_type=='dutch'], aes(x=Valence, y=Arousal, color='red')) +
      geom_segment(aes(x=-1.25, y=0, xend=1.25, yend=0), color='black', size=0.1) +
      geom_segment(aes(x=0, y=-1.25, xend=0, yend=1.25), color='black', size=0.1) +
      geom_path() +
      geom_path(data=dt_coords, aes(x=x_cord, y=y_cord), col='black', size=0.1) +
      ...
      geom_text(aes(x=dt_cord_4[52]$x_cord + 0.2, y=dt_cord_4[52]$y_cord, label='Content'), colour='black') +
      # Title
      geom_text(aes(x=0, y= -1.4, label=paste0('Auction Type: Dutch')), colour='black') +
      scale_x_continuous(limits = c(-1.5, 1.5)) +
      scale_y_continuous(limits = c(-1.5,1.5)) +
      theme_void() +
      theme(legend.position = 'none')
    fName <- paste0('~/Dropbox/pkg.data/auction_emotions/Figures_regression/Circles-_AuctionType: Dutch.pdf')
    ggsave(filename=fName, device='pdf')
    ...
    ggplot2::ggplot(dt_sprd[auction_type=='first_price'], aes(x=Valence, y=Arousal, color='blue')) +
      ...
      geom_text(aes(x=dt_cord_4[52]$x_cord + 0.2, y=dt_cord_4[52]$y_cord, label='Content'), colour='black') +
      # Title
      geom_text(aes(x=0, y= -1.4, label=paste0('Auction Type: First Price')), colour='black') +
      scale_x_continuous(limits = c(-1.5, 1.5)) +
      scale_y_continuous(limits = c(-1.5,1.5)) +
      theme_void() +
      theme(legend.position = 'none')
    fName <- paste0('~/Dropbox/pkg.data/auction_emotions/Figures_regression/Circles-_AuctionType: First Price.pdf')
    ggsave(filename=fName, device='pdf')
