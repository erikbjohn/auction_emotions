reg_auction <- function(){
  ...
  if(!file.exists(reg_feedback_location)){
    ...
    dt_auction <- dt_scores[MarkerType %in% c('auction')]
    saveRDS(dt_auction, file='~/Dropbox/pkg.data/auction_emotions/Raw/dt_auction.rds')
    #dt_close <- dt_close[MarkerType %in% 'info']
    rm(dt_scores)
    #rm(dt_scores)
    emotions <- unique(dt_auction$EmotionType)
    iter <- 0
    if(!(file.exists('~/Dropbox/pkg.data/auction_emotions/Raw/reg_auction.rds'))){
      dt_emotions_reg <- data.table()
    } else {
      dt_emotions_reg <- readRDS('~/Dropbox/pkg.data/auction_emotions/Raw/reg_auction.rds')
    ...
    for(emotion in emotions){    
      iter <- iter + 1
      cat(emotion, '\n')
      dt_emotion <- dt_auction[EmotionType %in% emotion]
      dt_emotion <- dt_emotion[, dutch:=0]
      dt_emotion <- dt_emotion[AuctionType=='dutch', dutch:=1]
      dt_emotion <- dt_emotion[, time_elapsed := marker_time_elapsed]
      dt_emotion <- dt_emotion[, time_elapsed_x_dutch := time_elapsed*dutch]
      dt_emotion <- dt_emotion[, time_elapsed_x_value := time_elapsed*Value]
      dt_emotion <- dt_emotion[, time_elapsed_x_value_x_dutch := time_elapsed_x_dutch * Value]
      lm_mod <- lm(Score_num ~ dutch + Value + time_elapsed + time_elapsed_x_dutch +
                     #time_elapsed_x_value + time_elapsed_x_value_x_dutch +
                     participant_id,
                             data=dt_emotion)
      ...
      mat_summary <- sum_lm_mod$coefficients[2:5,c(1:3)]
...
#      setnames(dt_emotion_reg, 'value', emotion) 
      saveRDS(dt_emotions_reg, '~/Dropbox/pkg.data/auction_emotions/Raw/reg_auction.rds')
      rm(dt_emotion)
      rm(dt_emotion_reg)
      rm(lm_mod)
    }
    ...
    write.csv(dt_spread, '~/Dropbox/pkg.data/auction_emotions/Clean/reg_auction.csv')
    saveRDS(dt_spread, reg_auction_location)
  } else {
    l_reg <- readRDS(reg_auction_location)
  }
