reg_open <- function(){
  ...
  reg_open_location <- '~/Dropbox/pkg.data/auction_emotions/Clean/reg_open.rds'
  
  if(!file.exists(reg_open_location)){
    ...
    for(emotion in emotions){    
      ...
      f_test <- linearHypothesis(lm_mod, c('dutch = - value_x_dutch'))
      f_stat <- f_test$F[2]
      ...
      dt_emotion_reg <- rbindlist(list(dt_emotion_reg, dt_diag), use.names=TRUE)      
...
#      setnames(dt_emotion_reg, 'value', emotion)
    ...
    write.csv(dt_spread, '~/Dropbox/pkg.data/auction_emotions/reg_open.csv')
    saveRDS(dt_spread, reg_open_location)
  } else {
    l_reg <- readRDS(reg_open_location)
  }
